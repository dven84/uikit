    ...
    <?php foreach ($data['items'] as $i => $item) :
        ...
        if ($data['nav'] == 'thumbnav' && $src) {
            ...
            if (Uikit::isImage($src) == 'svg') {
                $thumbnail = Uikit::image($src, array_merge($attrs_thumbnail, ['width' => $data['thumbnav_width'], 'height' => $data['thumbnav_height']]));
            } elseif ($data['thumbnav_width'] || $data['thumbnav_height']) {
                $thumbnail = Uikit::image([$src, 'thumbnail' => [$data['thumbnav_width'], $data['thumbnav_height']], 'sizes' => '80%,200%'], $attrs_thumbnail);
            } else {
